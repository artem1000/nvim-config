vim.cmd("set titlestring=%{hostname()}\\ \\ %F\\ \\ %{strftime('%Y-%m-%d\\ %H:%M',getftime(expand('%')))}")
vim.cmd("set title")
...
--
-- navigate to vault
vim.keymap.set("n", "<leader>oo", ":cd /Users/agrinchenko/Documents/Notes/Notes<cr>")

-- convert note to template and remove leading white space
vim.keymap.set("n", "<leader>on", ":ObsidianTemplate note<cr> :lua vim.cmd([[1,/^\\S/s/^\\n\\{1,}//]])<cr>")
-- strip date from note title and replace dashes with spaces
-- must have cursor on title
vim.keymap.set("n", "<leader>of", ":s/\\(# \\)[^_]*_/\\1/ | s/\\(-\\|_\\)/ /g<cr>")
--
-- search for files in full vault
vim.keymap.set("n", "<leader>os", ':Telescope find_files search_dirs={"~/Documents/Notes/notes"}<cr>')
vim.keymap.set("n", "<leader>oz", ':Telescope live_grep search_dirs={"~/Documents/Notes/Notes/notes"}<cr>')
--
...
vim.keymap.set("n", "<leader>ok", ":!mv '%:p' ~/Documents/Notes/zettelkasten<cr>:bd<cr>")
